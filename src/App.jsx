...
const Home = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
      <section className="hero" style={{ padding: 'clamp(3rem,8vw,10rem) clamp(1rem,4vw,2rem)' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto', position: 'relative', zIndex: 2 }}>
          <motion.div
            initial={{ scale: 0.9, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            transition={{ delay: 0.2, duration: 0.5 }}
            style={{ marginBottom: '2rem', display: 'inline-flex', alignItems: 'center', gap: '0.8rem', padding: '0.5rem 1.5rem', background: 'rgba(255,255,255,0.1)', borderRadius: '100px', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.2)', flexWrap: 'wrap', justifyContent: 'center' }}
          >
            <Sparkles size={18} color="#fbbf24" />
            <span style={{ fontSize: '0.85rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Next-Gen Election Assistant</span>
          </motion.div>
        ...
        <div style={{ position: 'absolute', top: '10%', left: '5%', width: 'min(300px,40vw)', height: 'min(300px,40vw)', background: 'var(--primary-accent)', borderRadius: '50%', filter: 'blur(150px)', opacity: 0.2, zIndex: 0 }}></div>
        <div style={{ position: 'absolute', bottom: '10%', right: '5%', width: 'min(400px,50vw)', height: 'min(400px,50vw)', background: 'var(--secondary)', borderRadius: '50%', filter: 'blur(180px)', opacity: 0.1, zIndex: 0 }}></div>
      </section>
      ...
      <div className="main-content">
        <div style={{ marginBottom: 'clamp(4rem,8vw,8rem)' }}>
          ...
          <div className="card-grid">
            {journeySteps.map((step, i) => (
              ...
              >
                <div style={{ background: `${step.color}15`, width: '70px', height: '70px', borderRadius: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '2rem', color: step.color, border: `1px solid ${step.color}30` }}>
                  {React.cloneElement(step.icon, { size: 34 })}
                </div>
                <h3 style={{ fontSize: 'clamp(1.2rem,2.5vw,1.6rem)', marginBottom: '1rem' }}>{step.title}</h3>
        ...
        <div style={{ marginTop: 'clamp(4rem,8vw,8rem)', background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)', padding: 'clamp(3rem,6vw,6rem) clamp(1.5rem,4vw,4rem)', borderRadius: '40px', color: 'white', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'relative', zIndex: 2 }}>
            <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 1 }}>
              <Trophy size={64} color="var(--accent)" style={{ marginBottom: '2rem' }} />
              <h2 style={{ color: 'white', fontSize: 'clamp(1.8rem,4vw,3.5rem)', marginBottom: '1.5rem', maxWidth: '800px', margin: '0 auto 2rem', textAlign: 'center' }}>Level Up Your <span style={{ color: 'var(--accent)' }}>Civic IQ</span></h2>
              <p style={{ fontSize: 'clamp(1rem,2vw,1.3rem)', opacity: 0.8, marginBottom: '4rem', maxWidth: '700px', margin: '0 auto 4rem', textAlign: 'center' }}>Master the election process and earn exclusive digital badges.</p>
              ...
              <div className="card-grid" style={{ gap: '2rem' }}>
                {[
                  ...
                  <div key={i} style={{ background: 'rgba(255,255,255,0.05)', padding: 'clamp(1.5rem,3vw,2.5rem)', borderRadius: '30px', border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(10px)', textAlign: 'center' }}>
              ...
              <div style={{ textAlign: 'center', marginTop: '3rem' }}>
                <Link to="/quiz" className="btn btn-primary" style={{ background: 'var(--accent)', padding: '1.2rem 4rem', fontSize: '1rem' }}>Start Certification Quiz</Link>
              </div>
            </motion.div>
          </div>
        ...
        <div style={{ marginTop: 'clamp(4rem,8vw,8rem)', background: 'white', padding: 'clamp(2rem,5vw,5rem)', borderRadius: '40px', boxShadow: 'var(--shadow-lg)', border: '1px solid var(--border)' }}>
          <h2 className="section-title">Truth & Clarity</h2>
          <div className="fact-grid">
            <motion.div whileHover={{ scale: 1.02 }} style={{ background: '#fef2f2', padding: 'clamp(1.5rem,3vw,3rem)', borderRadius: '30px', border: '1px solid #fee2e2' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', color: 'var(--error)', fontWeight: 800, marginBottom: '1.5rem', fontSize: '1.1rem', flexWrap: 'wrap' }}>
                <AlertTriangle size={28} /> POPULAR MYTH
              </div>
              <p style={{ fontSize: 'clamp(1rem,2vw,1.4rem)', fontWeight: 700, lineHeight: '1.4', color: '#991b1b' }}>"Your vote is not secret and can be traced back to you by the government."</p>
            </motion.div>
            <motion.div whileHover={{ scale: 1.02 }} style={{ background: '#ecfdf5', padding: 'clamp(1.5rem,3vw,3rem)', borderRadius: '30px', border: '1px solid #d1fae5' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', color: 'var(--success)', fontWeight: 800, marginBottom: '1.5rem', fontSize: '1.1rem', flexWrap: 'wrap' }}>
                <CheckCircle2 size={28} /> VERIFIED FACT
              </div>
              <p style={{ fontSize: 'clamp(1rem,2vw,1.4rem)', fontWeight: 700, lineHeight: '1.4', color: '#065f46' }}>"The secrecy of the ballot is legally protected. No machine or official can identify who you voted for."</p>
            </motion.div>
          </div>
        </div>
      </div>
    ...
    <div className="app-container" style={isChatPath ? { height: '100vh', overflow: 'hidden', position: 'fixed', width: '100%', top: 0, left: 0 } : {}}>
        ...
        {mobileMenuOpen && (
          <motion.div
            ...
            style={{
              ...
              height: '100%', background: '#0f172a', zIndex: 2000,
              display: 'flex', flexDirection: 'column', padding: '5rem 2rem 2rem',
              gap: '0.5rem', overflowY: 'auto', boxShadow: '-10px 0 30px rgba(0,0,0,0.3)'
            }}
          >
            <button
              onClick={() => setMobileMenuOpen(false)}
              style={{ position: 'absolute', top: '1.2rem', right: '1.2rem', background: 'rgba(255,255,255,0.1)', border: 'none', color: 'white', cursor: 'pointer', padding: '0.6rem', borderRadius: '50%', display: 'flex' }}
            >
              <X size={20} />
            </button>
            ...
            {navLinks.map(link => (
              <Link
                ...
                onClick={() => setMobileMenuOpen(false)}
                style={{
                  color: location.pathname === link.to ? 'var(--primary-accent)' : 'white',
            ...
            <div style={{ marginTop: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {/* Language picker */}
              <div style={{ background: 'rgba(255,255,255,0.07)', borderRadius: '12px', padding: '0.5rem' }}>
                {languages.map(l => (
                  <div
                    key={l.name}
                    onClick={() => { setLanguage(l.name); setMobileMenuOpen(false); }}
                    style={{ padding: '0.6rem 1rem', borderRadius: '8px', cursor: 'pointer', color: language === l.name ? 'var(--primary-accent)' : 'rgba(255,255,255,0.7)', fontWeight: language === l.name ? 700 : 500 }}
                  >
                    {l.name}
                  </div>
                ))}
      ...
      {!isAdminPath && !isChatPath && (
        <nav className="navbar" style={{ padding: scrolled ? '0.8rem 5%' : '1.5rem 5%', boxShadow: scrolled ? 'var(--shadow)' : 'none' }}>
          <Link to="/" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '1rem', flexShrink: 0 }}>
            <div style={{ background: 'var(--primary)', padding: '0.6rem', borderRadius: '12px' }}>
              <Vote size={28} color="white" />
            </div>
            <span style={{ fontSize: 'clamp(1.3rem,3vw,1.8rem)', fontWeight: 900, color: 'var(--primary)', letterSpacing: '-0.04em' }}>
          ...
          <div className="nav-links">
            {navLinks.map(link => (
              <Link key={link.to} to={link.to} style={{ textDecoration: 'none', color: location.pathname === link.to ? 'var(--primary-accent)' : 'var(--text)', fontWeight: 700, fontSize: '0.95rem', whiteSpace: 'nowrap' }}>
                {link.label}
              </Link>
            ))}
          </div>
          ...
          <div className="nav-actions">
            <div style={{ position: 'relative' }}>
              ...
              {showLangMenu && (
                <div style={{ position: 'absolute', top: '120%', right: 0, background: 'white', border: '1px solid var(--border)', borderRadius: '12px', padding: '0.5rem', boxShadow: 'var(--shadow-lg)', zIndex: 1100, minWidth: '150px', maxHeight: '300px', overflowY: 'auto' }}>
                  {languages.map(l => (
                    <div key={l.name} onClick={() => { setLanguage(l.name); setShowLangMenu(false); }} style={{ padding: '0.6rem 1rem', borderRadius: '8px', cursor: 'pointer', background: language === l.name ? 'rgba(59,130,246,0.1)' : 'transparent', color: language === l.name ? 'var(--primary-accent)' : 'var(--text)', fontWeight: language === l.name ? 700 : 500 }}>
                      {l.name}
                    </div>
                  ))}
                </div>
            ...
            <button className="btn btn-primary" style={{ borderRadius: '14px', padding: '0.8rem 1.8rem', display: 'flex', alignItems: 'center', gap: '0.8rem' }} onClick={user ? handleLogout : () => setIsAuthModalOpen(true)}>
              {user ? (<><img src={user.photoURL} alt={user.displayName} style={{ width: '24px', height: '24px', borderRadius: '50%' }} />{t.logout}</>) : (<><LogIn size={18} /> {t.joinNow}</>)}
            </button>
          </div>
      ...
      {!isAdminPath && !isChatPath && (
        <footer style={{ background: 'var(--primary)', color: 'white', padding: 'clamp(3rem,8vw,8rem) 5% clamp(2rem,4vw,4rem)' }}>
            ...
            <div>
              ...
              <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {[['Registration', '/process'], ['Booth Maps', '/booth'], ['AI Profiles', '/candidates'], ['Certification', '/quiz']].map(([label, path]) => (
                  <li key={path}><Link to={path} style={{ color: 'white', textDecoration: 'none', opacity: 0.6 }}>{label}</Link></li>
                ))}
              </ul>
            </div>
            <div>
              ...
              <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {['Helpline 1950', 'Complaints', 'Voter FAQ', 'Accessibility'].map(label => (
                  <li key={label}><Link to="/help" style={{ color: 'white', textDecoration: 'none', opacity: 0.6 }}>{label}</Link></li>
                ))}
              </ul>
            </div>
            <div style={{ background: 'rgba(255,255,255,0.05)', padding: 'clamp(1.5rem,3vw,2.5rem)', borderRadius: '30px', border: '1px solid rgba(255,255,255,0.1)' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                <input type="email" placeholder="email@voter.in" style={{ flex: 1, minWidth: '140px', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', padding: '0.8rem 1rem', borderRadius: '12px', color: 'white' }} />
